...
def rolling_predictions(target_name, data, ini_date='2020-03-01', split=0.75, horizon_forecast=14, maxlag=15):
    ...
    X_train, X_test, y_train, y_test = train_test_split(df_lag, target, train_size=split, test_size=1 - split,
                                                        shuffle=False)
...
def make_forecast_all_cantons(target_curve_name, predictors, vaccine=True, smooth=True, ini_date='2020-03-01',
                              title=None):
    '''
...
def save_to_database(df, table_name):
    ...
    logger.info(f'Forecast Data  "{table_name}" saved to DB')
